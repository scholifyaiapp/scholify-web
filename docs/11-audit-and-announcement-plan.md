...
- **`/api/health`** → `status: ok`, `billing: "live"` — Stripe is the exclusive billing rail; Supabase url + anon + service role set; PostHog on.
- **Fresh signup** created a real Supabase account, and the app's `start-trial` path (`src/lib/auth.tsx` → `api/stripe?action=start-trial`) granted the Pro trial server-side through the service role.
