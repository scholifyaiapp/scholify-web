...
/*
 * Scholify — the ORIGINAL seed question bank (waves 1) for the nine papers that
 * had one before the per-paper wave files landed: BT, MA, FA, LW, PM, TX, FR,
 * AA, FM. The six Strategic Professional papers seed entirely from their own
 * wave files and never load this module.
 *
 * Split out of acca-content.ts so paper METADATA (PAPERS) can stay eager while
 * the question CONTENT is fetched per paper, on demand. Loaded only through
 * acca-paper-content.ts — never import it from a page or component.
 *
 ...
 * or marking schemes (their IP).
 */
export const CORE_QUESTIONS: AccaQuestion[] = [
