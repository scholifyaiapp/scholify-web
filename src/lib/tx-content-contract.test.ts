...
describe("TX/F6 complete content contract", () => {
  it("has the correct A/B/C exam structure", () => {
    expect(examBlueprint("TX")?.sections.map((section) => section.id)).toEqual(["A", "B", "C"])
  })
   ...
   */
    const questions = getPracticeInventory("TX")
    expect(questions).toHaveLength(TX_CONTENT_TARGET.sectionA)
    expect(getQuestions("TX").some((q) => q.recall)).toBe(false)
    expect(new Set(questions.map((q) => q.id)).size).toBe(questions.length)
    expect(new Set(questions.map((q) => q.stem.trim().toLowerCase())).size).toBe(questions.length)
  })
  it("serves 70 complete Section B cases with 350 linked questions", () => {
    const cases = getOtCases("TX")
    const questions = cases.flatMap((item) => item.questions)
    expect(cases).toHaveLength(TX_CONTENT_TARGET.sectionBCases)
    expect(questions).toHaveLength(TX_CONTENT_TARGET.sectionBQuestions)
    expect(new Set(questions.map((q) => q.id)).size).toBe(questions.length)
    expect(new Set(questions.map((q) => q.stem.trim().toLowerCase())).size).toBe(questions.length)
  ...
  it("serves 50 Section C cases and 150 flashcards", () => {
    expect(getWrittenQuestions("TX")).toHaveLength(TX_CONTENT_TARGET.sectionC)
