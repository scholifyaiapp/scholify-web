...
import { markRetentionMilestone } from "@/lib/retention"
...
export default function Dashboard() {
  ...
  const [markGlow, setMarkGlow] = useState(false)

  // Refs for the streak-at-risk scroll target and the first-visit tour.
  const markCompleteRef = useRef<HTMLButtonElement | null>(null)
  const taskCardRef = useRef<HTMLDivElement | null>(null)
  const laraCardRef = useRef<HTMLDivElement | null>(null)

  useEffect(() => {
    if (shouldShowWelcomeChecklist()) setShowWelcome(true)
  }, [])

  // First-visit tour — only when the welcome checklist isn't already taking over.
  useEffect(() => {
    try {
      if (window.localStorage.getItem("scholify_tutorial_done") === "true") return
    } catch {
      return
    }
    if (shouldShowWelcomeChecklist()) return
    const t = window.setTimeout(() => setShowTour(true), 1000)
    return () => window.clearTimeout(t)
  }, [])

  // Auto-dismiss the Day-3 card after 8s.
  useEffect(() => {
    if (!showDay3) return
    const t = window.setTimeout(() => setShowDay3(false), 8000)
    return () => window.clearTimeout(t)
  }, [showDay3])
  ...
  const handleComplete = useCallback(async () => {
    ...
    // Retention milestones: first task, Day-3 retained, Day-7 retained.
    markRetentionMilestone({
      user,
      streak: next.streak,
      isFirstTask: progress.completed.length === 0,
    })

    // Day 3 — the first major dropout cliff. Celebrate crossing it.
    if (next.streak === 3) {
      window.setTimeout(() => setShowDay3(true), 1500)
    }
  ...
  // per day so the dashboard doesn't re-call the API on every mount. When the
  // API is unavailable we fall back to a smart, specific local line — never
  // a generic "Amazing work!".
  const laraFallback = getLaraFallback({
    name: firstName,
  ...
  // Streak at risk: after 6pm, not done today, and a live streak to protect.
  const streakAtRisk = !isDone && progress.streak > 0 && hour >= 18

  const scrollToComplete = useCallback(() => {
    markCompleteRef.current?.scrollIntoView({ behavior: "smooth", block: "center" })
    setMarkGlow(true)
    window.setTimeout(() => setMarkGlow(false), 2000)
  }, [])
  ...
  return (
    <DashboardLayout>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        ...
        {/* Streak-at-risk banner — appears after 6pm if today isn't done yet */}
        <AnimatePresence>
          {streakAtRisk && (
            <motion.div
            ...
            >
              ...
              >
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#FF9F0A" }}>
                    ⚠️ Streak at risk
                  </div>
                  <div style={{ fontSize: 12, color: "rgba(255,159,10,0.7)", marginTop: 2 }}>
                    {progress.streak}-day streak ends at midnight.
                  </div>
                </div>
                <motion.button
                  type="button"
      ...
      <Day3Card open={showDay3} onClose={() => setShowDay3(false)} />

      {/* First-visit tour — points at task, Mark Complete, and Lara */}
      {showTour && (
        <FirstVisitTour
          steps={
            [
              {
                ref: taskCardRef,
                title: "👆 This is your task for today.",
                body: `It takes exactly ${task.estimated_minutes || dailyMinutes} minutes.`,
                cta: "Next →",
              },
              {
                ref: markCompleteRef,
                title: "When you're done, tap this.",
                body: "Your streak starts with one tap.",
                cta: "Got it ✓",
              },
              {
                ref: laraCardRef,
                title: "Lara sends you a personal message every day.",
                body: "She knows your name, your streak, and your goal.",
                cta: "Let's start →",
              },
            ] as TourStep[]
          }
          onDone={() => {
            setShowTour(false)
            try {
              window.localStorage.setItem("scholify_tutorial_done", "true")
            } catch {
              /* ignore */
            }
          }}
